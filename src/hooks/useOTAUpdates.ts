...
export async function checkAndApplyUpdate(isManual: boolean = false): Promise<OTACheckResult> {
  ...
  try {
    ...
    if (result.isAvailable) {
      ...
      Alert.alert(
        '✨ Update Ready',
        'A new version of RehearsalHub Admin has been downloaded. Restart now to apply the latest updates.',
        [
          {
            text: 'Later',
            style: 'cancel',
      ...
      if (isManual) {
        Alert.alert('Up to Date', 'You are already running the latest version of RehearsalHub Studio.');
      }
...
export function useOTAUpdates() {
  ...
  const runBackgroundCheck = async () => {
    ...
    const now = Date.now();
    if (now - lastCheckedAt.current < CHECK_INTERVAL_MS) {
      return;
    }
    ...
    try {
      ...
      if (update.isAvailable) {
        ...
        await Updates.fetchUpdateAsync();
        console.log('[OTA] Auto-download complete — will prompt user.');
        Alert.alert(
          '✨ Update Ready',
          'A new version of RehearsalHub Studio has been downloaded. Restart now to apply the latest updates.',
          [
